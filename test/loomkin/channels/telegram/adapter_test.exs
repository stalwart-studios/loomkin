defmodule Loomkin.Channels.Telegram.AdapterTest do
  ...
  describe "parse_inbound/1" do
    ...
    test "parses callback query with structured ask_user data" do
      ...
      assert {:callback, "q-42", "ask_user:q-42:1"} = Adapter.parse_inbound(update)
    end
    ...
    test "parses callback query with legacy unstructured data" do
      ...
      # Legacy fallback: data is used as-is for both callback_id and data
      assert {:callback, "Option A", "Option A"} = Adapter.parse_inbound(update)
    end
