defmodule Loom.Teams.ModelRouter do
  @moduledoc """
  Selects the model for each agent/task combination and handles opt-in escalation.

  ## Philosophy

  Loom's team architecture relies on fluid OTP communication, peer review, and
  ...
  2. The user's configured default model (`[model].default` in `.loom.toml`)

  ## Escalation (Opt-in)

  Escalation only activates when `[teams.models].escalation` is configured in
  `.loom.toml` as an ordered list of model strings. When absent, `escalate/1`
  ...
  2. User's configured default model (`Loom.Config.get(:model, :default)`)
  """
  ...
  Escalation is opt-in. Configure it in `.loom.toml`:
  ...
  Reads `[model].default` from `.loom.toml` via `Loom.Config`, falling back
  ...
  Kept for backward compatibility — reads `[teams.models]` from `.loom.toml`
